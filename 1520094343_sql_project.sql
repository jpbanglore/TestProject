...
SELECT CONCAT(CONCAT(name, ' ', monthlymaintenance) , ' ' , 'expensive') AS result
FROM Facilities
WHERE monthlymaintenance > 100.0 
UNION
SELECT CONCAT(CONCAT(name, ' ', monthlymaintenance),' ', 'cheap') 
FROM Facilities
WHERE monthlymaintenance < 100.0
...
FROM Bookings b
JOIN Facilities f
JOIN Members m ON b.facid = f.facid
AND b.memid = m.memid
WHERE b.facid =0
OR b.facid =1
...
SELECT CONCAT(f.name, ' ' , m.firstname) AS booking, 
